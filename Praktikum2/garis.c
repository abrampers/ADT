...
/* Realisasi ADT GARIS, menggunakan ADT point */
...
#define epsilon 1e-6
#define f_equal(A,B) abs(A - B) < epsilon
...
/* Dua garis saling tegak lurus jika hasil perkalian kedua gradiennya = -1 */
{
	...
	if (f_equal(Absis(PAwal(L1)), Absis(PAkhir(L1)))) {
		/* gradien garis 1 tidak dapat dihitung */
		return (f_equal(Ordinat(PAwal(L2)), Ordinat(PAkhir(L2))));
	} else if (f_equal(Absis(PAwal(L2)), Absis(PAkhir(L2)))) {
    /* gradien garis 2 tidak dapat dihitung */
    return (f_equal(Ordinat(PAwal(L1)), Ordinat(PAkhir(L1))));
  } else {
		/* gradien dapat dihitung */
		return f_equal(Gradien(L1) * Gradien(L2), -1.00);
	}
}
...
/* Dua garis saling sejajar jika memiliki gradien yang sama */
{
	...
	if (f_equal(Absis(PAwal(L1)), Absis(PAkhir(L1))) ||
	    f_equal(Absis(PAwal(L2)), Absis(PAkhir(L2)))) {
		/* gradien tidak dapat dihitung */
		return f_equal(Absis(PAwal(L1)), Absis(PAkhir(L1))) &&
           f_equal(Absis(PAwal(L2)), Absis(PAkhir(L2)));
	} else {
		/* gradien dapat dihitung */
		return f_equal(Gradien(L1), Gradien(L2));
	}
}
