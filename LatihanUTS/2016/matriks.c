...
void findElement(MATRIKS M, ElType X, int * brs, int * kol)
{
	boolean found = false;
	int i, j;
...
MATRIKS subMatriks(MATRIKS M, int RDel, int CDel)
{
    MATRIKS T;
    int idxRowT = 1, idxColT = 1;
    int i, j;
    MakeMATRIKS(NBrsEff(M) - 1, NKolEff(M) - 1, &T);
    for (i = GetFirstIdxBrs(M); i <= GetLastIdxBrs(M); i++) {
        idxColT = 1;
        for (j = GetFirstIdxKol(M); j <= GetLastIdxKol(M); j++) {
            if (i != RDel && j != CDel) {
                Elmt(T, idxRowT, idxColT) = Elmt(M, i, j);
                idxColT++;
            }
        }
        if (i != RDel) {
