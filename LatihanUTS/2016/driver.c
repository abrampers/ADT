...
int main(int argc, char const *argv[]) {
    ...
    int x, y, target;
    scanf("%d", &target);
    findElement(M1, target, &x, &y);
    printf("\n%d Ditemukan pada indeks baris = %d, kolom = %d", target, x, y);

    printf("\nSOAL b\n");
    printf("\nMasukkan Baris yang dihapus : ");
    scanf("%d", &x);
    printf("\nMasukkan Kolom yang dihapus : ");
    scanf("%d", &y);
    printf("Matriks dihapus : \n");
    TulisMATRIKS(subMatriks(M1, x, y));
